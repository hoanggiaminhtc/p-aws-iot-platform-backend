...
def on_localmessage(client, userdata, message):
    ...
    if (mess["message"] == "send data"):
        ...
        clientcloud.publish(message.topic, dt, qos=1)
    ...
    if (mess["message"] == "Last will"):
        if (mess["value"] == "Device Disconnected"):
            clientcloud.publish(message.topic, dis, qos=1)
        elif (mess["value"] == "Device Connected"):
            clientcloud.publish(message.topic, connect, qos=1)
def on_cloudmessage(client, userdata, message):
    ...
    # When user add new device
        ...
        with open('controlDVtopic.csv', 'a') as f:
            ...
            writer = csv.writer(f)
            writer.writerow(row)
        clientcloud.publish(resT, messjson, qos = 1)
...
clientlocal.subscribe("test")
clientcloud.subscribe("test")
header_list = ["topic"]
